...
import { getAllDeptLoans } from "../../services/ManagerService";
...
export function OnDeptLoans() {
  const { classes } = useStyles();
  const [activePage, setPage] = useState(1);
  const { data } = useQuery(["getAllDeptLoans", activePage], () => getAllDeptLoans(activePage), { keepPreviousData: true });
  ...
  return (
    <Container>
      ...
      <Pagination size="lg" page={activePage} onChange={setPage} total={Math.ceil(data.total / 20)} withEdges />
    </Container>
  );
}
